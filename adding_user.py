...
LOGIN_URL = "https://cybersec.hackerearth.com/login"
ENDPOINT_URL = 'https://cybersec.hackerearth.com/api/v1/users'
client = requests.session()
csrftoken = re.search("var\s+csrf_nonce\s+=\s+(.*);", client.get(LOGIN_URL).text).group(0)[18:-2]
login_data = {"name":"Admin", "password":"5f4dcc3b5aa765d61d8327deb882cf99", "nonce":csrftoken}
r1 = client.post(LOGIN_URL, data=login_data)
for i in range(len(usernames)):
    csrftoken = re.search("var\s+csrf_nonce\s+=\s+(.*);", client.get("https://cybersec.hackerearth.com/admin/users").text).group(0)[18:-2]
